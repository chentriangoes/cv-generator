...
function ExperienceInfo(props) {
  const { data, deleteExpInfo, handleSubmit, reorderList } = props;
  ...
  const submittedInfoMarkup = data.map((submittedInfo, idx) => (
    <div
  ...
  const addlInfoMarkup = expInfo.additionalInfo.map((item, idx) => (
    <div
      key={item.id}
      data-index={idx}
      draggable
